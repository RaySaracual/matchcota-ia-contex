...
1. [ ] Backend: implementar `ChatHub` con SignalR.
...
2. [ ] Backend: endpoints REST de chat.
...
3. [ ] Frontend: integrar paquete `signalr_netcore`.
...
4. [ ] Frontend: `ConversationPage` — sala de chat en tiempo real.
...
5. [ ] Frontend: navegar a `ConversationPage` desde `MatchesPage` al tocar un match.
6. [ ] Tests: ChatService (autorizacion de sala, persistencia de mensaje) + widget ConversationPage.

### Multimedia (BL-002 foto de perro)

7. [ ] Backend: `POST /api/v1/dogs/{dogId}/media` — multipart upload.
...
8. [ ] Frontend: selector de imagen en `ProfilePage` con `image_picker`.
...
9. [ ] Backend: `docker-compose.yml` en raiz del repositorio.
...
10. [ ] Backend: global exception middleware + Serilog.
...
11. [ ] Frontend: manejo de token expirado.
